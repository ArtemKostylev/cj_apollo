...
function useProvideMidtermExam() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<MidtermExam | undefined>(undefined);
  ...
  const onRowClick = useCallback((record: MidtermExam) => setSelectedRecord(record), [])
  ...
  const [{loading, error, data, refetch}, modifyMidtermExam, addMidtermExam, removeMidtermExam] = useApollo<MidtermExam>(
    FETCH_MIDTERM_EXAMS, {
      teacherId: versions[year].id, year, typeId: type, dateGte: dateGte.format(), dateLte: dateLte.format()
    },
    'fetchMidtermExams'
  );
