...
export type UpdateQuarterData = (args: {
    row: number;
    column: string;
    value: string;
}) => boolean;
export type UpdateData = (args: {
    row: number;
    column: number;
    value: string;
    group?: number;
}) => boolean;
export type UpdateDates = (args: {
    date: Moment;
    column: number;
    group: number;
}) => void;
...
export default function Journal() {
    ...
    const userCourses: Course[] = useMemo(
        () =>
            location.state?.versions[currentYear].courses ||
            auth.user?.versions[currentYear].courses,
        [location.state, auth.user, currentYear]
    );

    const parsedDates = useMemo(
        () => getDatesFromMonth(month, currentYear),
        [month, currentYear]
    );
    ...
    const onPeriodChange = useCallback(
        (period: Periods) => setPeriod(period),
        []
    );
        ...
        studentData = [
            ...studentData.slice(0, studentIndex),
            {
                ...
                quaterMark: [
                    ...
                    ...studentData[studentIndex].quaterMark.slice(
                        markIndex + 1
                    ),
                ],
            },
    ...
    let { loading, data, error, refetch, networkStatus } = useQuery<{
        fetchJournal: TeacherCourseStudent[];
    }>(FETCH_JOURNAL_QUERY, {
        variables: {
            teacherId:
                location.state?.versions[currentYear].id ||
    ...
    let dates_by_group = groupedData.map((group) => {
        ...
        const mappedDates = new Map(
            MONTHS_IN_PERIODS[period].map((it) => [it, [] as DateByGroup[]])
        );
        ...
        mappedDates.forEach((value, key) => {
            ...
            result.push(
                ...value,
                ...times(emptyCount, () => ({ date: undefined, month: key }))
            );
        });
    ...
    const updateDates: UpdateDates = ({ date, column, group }) => {
        ...
        students = students.filter((student) =>
            studentData
                ...
                ?.journalEntry.find((mark) =>
                    moment(mark.date).isSame(oldDate, "days")
                )
        );

        students.forEach((studentIndex) => {
            const student = studentData.find(
                (item) => item.id === studentIndex
            );
            if (!student)
                throw new Error(`Can't find student with id ${studentIndex}`);
            const marks = student.journalEntry;
            const cell = marks.find((el) =>
                moment(el.date).isSame(oldDate, "days")
            );
            ...
            if (!cell.delete_flag) {
                studentData = [
                    ...studentData.slice(0, studentId),
                    {
                        ...studentData[studentId],
                        journalEntry: [
                            ...studentData[studentId].journalEntry.slice(
                                0,
                                index
                            ),
                            {
