...
export const Table = memo(<T extends PrimitiveCacheEntity>({
                                                             fixed,
                                                             rowInfo,
                                                             controlsConfig,
                                                             updateQuery,
  ...
  const [{data, loading}, modifyEntity, addToQuery] = useApollo(fetchQuery, variables, dataKey);
  ...
  return (
    <TableWrapper fixed={fixed}>
      ...
      {Object.values(data).map((it) => (
        <TableRow key={it.id} item={it} updateMutation={updateQuery} rowInfo={rowInfo} refetchQueries={[{query: fetchQuery}]}
                  selected={it.id === selected} modifyEntity={modifyEntity} onRowClick={() => setSelected(it.id)} typeName={typeName}/>
      ))}
    </TableWrapper>
  )
})